...
class UNetExperiment:
    """
    This class implements the basic life cycle for a segmentation task with 
    ...
    def __init__(self, config, split, dataset):
        ...
        # We are using a standard cross-entropy loss since the model output is 
        # essentially a tensor with softmax'd prediction of each pixel's 
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # shape [BATCH_SIZE, 1, PATCH_SIZE, PATCH_SIZE] into variables data 
            # and target. 
            # Feed data to the model and feed target to the loss function
            
            data = batch["image"]
            target = batch["seg"]
            data, target = ((Variable(data).to(self.device, dtype=torch.float)), Variable(
                target).to(self.device))

            prediction = self.model(data)

            # We are also getting softmax'd version of prediction to output a 
            # probability map o that we can see how the model converges 
            # to the solution
            ...
            # ANSWER: First dimension -- Batch Index, 
            #         Second Dimension -- Class Probability , 
            #         Third Dimension -- y pixel, 
            #         Fourth Dimension -- z pixel
    ...
    def run_test(self):
        ...
        # We will do inferences on full 3D volumes, much like we will be doing
        # it when we deploy the model in the clinical environment.
